...
- 🐛 **修复识别结果没有输出问题**
  - 问题：服务端推理需要 8+ 秒（RTF=13.955），但客户端只等待 1 秒就断开连接
  - 修复：`idle_timeout` 从 1 秒增加到 10 秒，`max_wait_time` 从 5 秒增加到 15 秒
  - 效果：现在能正常接收识别结果

- 🐛 **修复噪音误触发问题**
  - 问题：VAD mode=2（中等严格度）对某些噪音敏感，导致没说话也触发识别
  - 修复：VAD mode 从 2 提高到 3（最严格模式）
  - 效果：减少误触发，更适合嘈杂环境

**待优化：MPS GPU 加速**
- 当前使用 CPU 推理，RTF=13.955（处理 1 秒音频需要 13.955 秒）
- 需要在 FunASR 服务端（claudeCodeFunasr 项目）启用 MPS 加速
- 启用后预期 RTF < 1，实现真正的实时识别
